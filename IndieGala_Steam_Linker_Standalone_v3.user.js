// ==UserScript==
// @name         IndieGala Steam Linker (Standalone v3)
// @namespace    https://github.com/gbzret4d/indiegala-steam-linker-v3
// @version      3.0.0
        ...
        bundles.forEach(bundle => {
            ...
            queueRequest(() => new Promise(resolve => {
                GM_xmlhttpRequest({
                    ...
                    onload: (res) => {
                        try {
                            // Quick regex scan for AppIDs in the response HTML
                            // We look for known Steam link patterns
                            // const steamLinks = res.responseText.match(/store\.steampowered\.com\/app\/(\d+)/g);
                            // Better: Look for game titles or verify if we can extract IDs.
                            // Since we can't reliably get IDs from regex alone without titles, 
                            // let's try to match existing CACHED titles if possible, or just look for "Owned" markers if IG had them? No.

                            // Alternative: Search for specific bundle text matches or IDs?
                            // Actually, simpler: Search for ANY appID that is in our Wishlist/Owned list
                            // This is a "fuzzy" check but effective for "Does this bundle have wishlisted items?"

                            // Let's iterate user's wishlist and check if any ID appears in the response
                            // This is computationally expensive if wishlist is huge, but efficient enough for text search.
                            ...
                            // Check IDs (Fastest)
                            // Use a Set for faster lookup if possible, but user arrays are small enough (<2000 usually)
                            // Optimization: extracting all IDs from page first
                            const pageIds = new Set();
                            const matches = text.matchAll(/store\.steampowered\.com\/app\/(\d+)/g);
                            for (const m of matches) pageIds.add(parseInt(m[1]));

                            // Also try to find IndieGala's internal "bundle-page-tier-item-image" sources which might not have steam links yet?
                            // Actually, most bundle pages DO have the Steam link in the overlay or meta.
